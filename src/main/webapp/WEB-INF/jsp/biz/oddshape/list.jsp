...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="oddshape/list" method="post" name="userForm" id="userForm">
			<table>
				<tr>
					<td>
					...
					<c:when test="${not empty userList}">
						<c:forEach items="${userList}" var="user" varStatus="vs">
							<tr>
								...
								<td>${user.point}</td>
								<td>
										<c:if test="${user.type== 1}">APP设置</c:if>
										<c:if test="${user.type == 2}">走圈</c:if>
										<c:if test="${user.type == 3}">导入</c:if>
								</td>
								
								<td>${user.createtime}</td>
								<td style="width: 60px;">
									<div class='hidden-phone visible-desktop btn-group'>
										<shiro:hasPermission name="oddshape:update">
										<a class='btn btn-mini btn-info' title="编辑" onclick="edit('${user.id }');"><i class='icon-edit'></i></a>
										</shiro:hasPermission>
										<shiro:hasPermission name="oddshape:delete">
										<a class='btn btn-mini btn-danger' title="删除" onclick="del('${user.id }','${user.name }');"><i class='icon-trash'></i></a>
										</shiro:hasPermission>
									</div>
								</td>
							</tr>
		...
		<table style="width:100%;">
			<tr>
				<td style="vertical-align:top;">
					<shiro:hasPermission name="oddshape:create">
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>oddshape/toAdd';
			 diag.Width = 222;
			 diag.Height = 380;
		...
		function edit(id){
			 ...
			 diag.Drag=true;
			 diag.Title ="资料";
			 diag.URL = '<%=basePath%>oddshape/toEdit?id='+id;
			 diag.Width = 222;
			 diag.Height = 380;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
		...
		function del(id,msg){
			bootbox.confirm("确定要删除["+msg+"]吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>oddshape/delete?id="+id;
